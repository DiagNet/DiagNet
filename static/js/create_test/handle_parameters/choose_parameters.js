/* Handles Parameters */
const chooseParametersContainer = document.getElementById(
  "chooseParametersContainer",
);
const submitParametersButton = document.getElementById("submitParameters");
const parameterInfoContainer = document.getElementById(
...
function checkSubmitValidity(
  ...
  for (const key in validInputMap) {
    ...
    if (activationDependencyMap["ACTIVATION_STATE"]) {
      ...
      if (activationState === undefined)
        activationState = ParameterField.ACTIVATION_RESULT.ACTIVATE;
      else
        valueC =
          activationState === ParameterField.ACTIVATION_RESULT.ACTIVATE ||
          activationState === ParameterField.ACTIVATION_RESULT.UNKNOWN;
    }
  ...
  if (
    !parameters.every((item) =>
      item["parameter_info"].checkFieldSubmitValidity(),
    )
  ) {
    disableSubmit(submitButton);
    return;
...
function createParameterFields(parameter) {
  const datatypes = (
    Array.isArray(parameter["type"]) ? parameter["type"] : [parameter["type"]]
  ...
  else if (datatypes.includes("device"))
...
async function createAndSaveParameterFields(parameters) {
  for (const parameterInfo of parameters) {
    if (
      parameterInfo === null ||
      typeof parameterInfo !== "object" ||
      Array.isArray(parameterInfo)
    ) {
      throw new Error(
        "Parameters can only be defined as a map like structure: Consider changing the test definition",
      );
    }
...
function createMutuallyExclusiveHandler(
  ...
  function getParameterFieldForNames(allParameterFields, searchNames) {
    ...
    for (const param of allParameterFields) {
      ...
      if (!paramName)
        throw new Error(
          "No name found for given parameter: Consider adding the 'name' field in the test definition",
        );
      if (typeof paramName !== "string")
        throw new Error(
          "Parameter name has to be a string: Consider adding a valid 'name' field in the test definition",
        );
    ...
    if (foundNamesCount !== searchNames.size) {
      throw new Error(
        "Mutually-Exclusive-Binding parameter not found as an actual Parameter",
      );
    }
    return output;
  }
  ...
  for (const param of allMutuallyExclusiveParameters) {
    ...
    const parameterNamesToParameterFields = getParameterFieldForNames(
      parameters,
      fieldNames,
    );
    ...
    const fields = Array.from(
      fieldNames,
      (neighbor) => parameterNamesToParameterFields[neighbor]["parameter_info"],
    );
...
function createDatatypeHandler(parameters, validInputMap) {
  for (const parameterInfo of parameters) {
    ...
    if (!parameterType) {
      throw new Error(
        `No datatype found for parameter ${parameterName}: Consider adding the 'type' field in the test definition`,
      );
    }
    ...
    const validateResultBasedOnRequirement = {
      ...
      optional: (result) =>
        result === DATATYPE_RESULT.SUCCESS ||
        result === DATATYPE_RESULT.UNKNOWN,
    };
    ...
    parameterInfo["datatype_handler"] = async (event) => {
      ...
      validInputMap[parameterName] =
        validateResultBasedOnRequirement[requirement](result);
      if (event && previousCheck === validInputMap[parameterName])
        event.validInputChanged = true;
    };
  }
}
...
function createInputListeners(parameters) {
  for (const parameterInfo of parameters) {
    const field = parameterInfo["parameter_info"];
    const handlerNames = [
      "mutually_exclusive_handler",
      "datatype_handler",
      "activation_handler",
      "valid_submit_handler",
      "datatype_dropdown_handler",
    ];
    ...
    if (toTrigger) {
      parameter["activation_handler"] = async () => {
        toTrigger.forEach((item) => {
          ...
          activationDependencyMap["ACTIVATION_STATE"][item.parameter["name"]] =
...
function loadDatatypes(parameters) {
  for (const parameter of parameters) {
    ...
    const datatypes = Array.isArray(parameter["type"])
      ? parameter["type"]
      : [parameter["type"]];
    for (let i = 0; i < datatypes.length; i++) {
      const datatype = datatypes[i];
      if (datatype && datatype.constructor === Object) {
        datatypeObjects.push(
          Datatype.toDatatype(
            datatype["name"],
        ...
        throw new Error(
          "Unknown datatype format. (Class definition seems to be wrong?) " +
            datatype,
        );
      }
    }
...
async function showParameters(
  ...
  createMutuallyExclusiveHandler(
    parameters,
    mutually_exclusive_bindings,
    currentlyBlockedMap,
  );
...
async function selectTestClass(testClass) {
  ...
  settingUp = true;
  if (previousTestClass === testClass) {
    // Already loaded
  ...
  if (!Array.isArray(parameters.parameters)) {
    reset();
    throw new Error(
      "Given parameters have to be contained in a list: Consider adding a valid test definition",
    );
  }
