...
class ListField extends ParameterField {
  async createField() {
    ...
    if (!this.parameters.length)
      throwException(
        `List "${this.getDisplayName()} does not have any parameters.`,
      );
    ...
    this.addButton.addEventListener("mousedown", (event) => {
      // TODO remove if unnecessary after test
  ...
  badgeCountTooLow() {
    ...
    this.countInfo.innerHTML =
      "minimum length for this list is " + this.min_length;
  }
  ...
  receiveValuesFromChildren() {
    ...
    for (const value of this.parameters) {
      ...
      const isDevice =
        field instanceof SingleLineDeviceField &&
        allDevices.includes(fieldValue);
  ...
  getInfo(globalTestClass, infoContainer) {
    ...
    const container = listParameterInfo.content
      .cloneNode(true)
      .querySelector("div");
    ...
    container.querySelector(".infoTabDatatypeDescription").textContent =
      "A list of entries, each defining its child parameters";

    // Constraints
    if (this.min_length)
      container.querySelector(".list-min-length").textContent = Math.max(
        1,
        this.min_length,
      );
    if (this.max_length)
      container.querySelector(".list-max-length").textContent = this.max_length;
    ...
    for (const entry of entries) {
      entryContainer.appendChild(
        this.getEntrySingleLineInfoContainer(
          entry,
          emptyEntries,
          infoContainer,
          globalTestClass,
   ...
   */
  getEntrySingleLineInfoContainer(
    entry,
    emptyEntriesLabel,
    container,
    globalTestClass,
  ) {
    const listItemViewContainer = listItemSmallInfo.content
      .cloneNode(true)
      .querySelector("div");
    ...
    listItemViewContainer
      ...
      .addEventListener("click", () => {
        ...
        if (this.getValue().length === 0)
          emptyEntriesLabel.classList.remove("d-none");
  ...
  showListItemInfo(entry, container, globalTestClass) {
    ...
    container.innerHTML = "";
    const templateContainer = listItemPageInfo.content
      .cloneNode(true)
      .querySelector("div");

    templateContainer
      .querySelector("#returnToListView")
      .addEventListener("click", () => {
  ...
  displayListItemInfo(entry, containerToAddTo) {
    for (const [index, collection] of entry.entries()) {
      if (entry.length > 1 && index !== 0) {
        const indexCounterContainer = indexCounter.content
          .cloneNode(true)
          .querySelector("div");
      ...
      for (let [key, value] of Object.entries(collection)) {
        ...
        if (Array.isArray(value)) {
          // List View
          const templateContainer = listItem.content
            .cloneNode(true)
            .querySelector("div");
          const itemContainer =
            templateContainer.querySelector("#listItemContainer");
          ...
          // Default View
          containerToAddTo.appendChild(
            this.displayDefaultItemInfo(key, value, isDevice),
          );
        }
      }
    }
