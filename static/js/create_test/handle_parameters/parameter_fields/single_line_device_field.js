const singleLineInputTemplateForDevices = document.getElementById(
  "parameterInputTemplateForDevices",
);
...
class SingleLineDeviceField extends SingleLineInputField {
  ...
  changedFromEmptyToValue() {
    const evaluation =
      (this.field ? this.field.value.length === 1 : true) ||
      this.selectedDropdownItem; // Selecting Something does not trigger === 1
  ...
  handleDropdownInput(event) {
    ...
    if (
      this.uninitializedDeviceList ||
      this.isTriggeredByInputValidation(event)
    )
      return;
  ...
  handleDropdownKeyDown(e) {
    // Escape Datatype Validation Trigger
    if (this.uninitializedDeviceList || this.isTriggeredByInputValidation(e))
      return;
