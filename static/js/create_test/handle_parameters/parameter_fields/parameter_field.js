...
class ParameterField {
  ...
  constructor(parameter) {
    if (new.target === ParameterField)
      throw new Error(
        "Cannot instantiate abstract class ParameterField directly",
      );
  ...
  checkTriggerResult(mapKey, parameterName, value, activateOnMatch = true) {
    ...
    return (
      isMatch === activateOnMatch ? this.showField() : this.hideField(),
      isMatch === activateOnMatch
        ? ParameterField.ACTIVATION_RESULT.ACTIVATE
        : ParameterField.ACTIVATION_RESULT.DEACTIVATE
    );
  }
  ...
  handleActivationTrigger(parameterName, value) {
    let currentStateRequiredIf = this.checkTriggerResult(
      "required_if",
      parameterName,
      value,
      true,
    );
    let currentStateForbiddenIf = this.checkTriggerResult(
      "forbidden_if",
      parameterName,
      value,
      false,
    ...
    return currentStateForbiddenIf === ParameterField.ACTIVATION_RESULT.UNKNOWN
      ? currentStateRequiredIf
      : currentStateForbiddenIf;
  }
  ...
  changedFromEmptyToValue() {
    throw new Error(
      "changedFromEmptyToValue() must be implemented in subclass",
    );
  }
